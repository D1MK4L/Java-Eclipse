...
/**
 * The login window that the Admin can login
 * There is also the option to register usernames to
 * link into a user depending on the role of the user.
 *  
 * @author D1MK4L
 */
...
//		});
		...
		loginBtn.setBounds(93, 124, 90, 40);
		login.add(loginBtn);
		...
		btnNewButton_2.setBounds(261, 124, 103, 40);
...
//				}
				...
				try (Connection connection = DBUtil.getConnection();
					...
					ps.setString(3, role);
					ps.executeUpdate();
					JOptionPane.showMessageDialog(null, "One row affected", "User Inserted Successfully", JOptionPane.INFORMATION_MESSAGE);
		...
		CloseRegBtn = new JButton("Close");
		CloseRegBtn.setVisible(false);
		CloseRegBtn.setEnabled(false);
		CloseRegBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				Main.getLoginForm().setVisible(false);
				Main.getLoginForm().setEnabled(false);
		...
		CloseRegBtn.setEnabled(false);
		CloseRegBtn.setForeground(Color.BLUE);
		CloseRegBtn.setFont(new Font("Tahoma", Font.BOLD, 12));
		CloseRegBtn.setBounds(88, 154, 90, 40);
		register.add(CloseRegBtn);
